...
namespace MQTT
{
  ...
  // Connect class
  Connect::Connect(String cid) : Message(CONNECT),
                                 _clean_session(true),
                                 _clientid(cid),
  ...
  // ConnectAck class
  ConnectAck::ConnectAck(uint8_t *data, uint32_t length) : Message(CONNACK)
  {
    uint32_t pos = 0;
  ...
  // Publish class
  Publish::Publish(String topic, String payload) : Message(PUBLISH),
                                                   _topic(topic),
  ...
  Publish::Publish(String topic, const __FlashStringHelper *payload) : Message(PUBLISH),
                                                                       _topic(topic),
  ...
  Publish::Publish(uint8_t flags, uint8_t *data, uint32_t length) : Message(PUBLISH, flags),
                                                                    _payload(NULL), _payload_len(0),
                                                                    _payload_mine(false)
  {
    uint32_t pos = 0;
  ...
  Publish::Publish(String topic, payload_callback_t pcb, uint32_t length) : Message(PUBLISH),
                                                                            _topic(topic),
                                                                            _payload_len(length),
  ...
  Publish::Publish(uint8_t flags, Client &client, uint32_t remaining_length) : Message(PUBLISH, flags),
  ...
  // PublishAck class
  PublishAck::PublishAck(uint16_t pid) : Message(PUBACK)
  {
    _packet_id = pid;
  }

  PublishAck::PublishAck(uint8_t *data, uint32_t length) : Message(PUBACK)
  {
    uint32_t pos = 0;
    _packet_id = read<uint16_t>(data, pos);
  }
  ...
  // PublishRec class
  PublishRec::PublishRec(uint16_t pid) : Message(PUBREC)
  {
    _packet_id = pid;
  }

  PublishRec::PublishRec(uint8_t *data, uint32_t length) : Message(PUBREC)
  {
    uint32_t pos = 0;
    _packet_id = read<uint16_t>(data, pos);
  }
  ...
  // PublishRel class
  PublishRel::PublishRel(uint16_t pid) : Message(PUBREL)
  {
    _packet_id = pid;
  }

  PublishRel::PublishRel(uint8_t *data, uint32_t length) : Message(PUBREL)
  {
    uint32_t pos = 0;
    _packet_id = read<uint16_t>(data, pos);
  }
  ...
  // PublishComp class
  PublishComp::PublishComp(uint16_t pid) : Message(PUBREC)
  {
    _packet_id = pid;
  }

  PublishComp::PublishComp(uint8_t *data, uint32_t length) : Message(PUBCOMP)
  {
    uint32_t pos = 0;
    _packet_id = read<uint16_t>(data, pos);
  }
  ...
  Subscribe::Subscribe(String topic, uint8_t qos) : Message(SUBSCRIBE),
                                                    _buffer(NULL), _buflen(0)
  {
    _need_packet_id = true;
  ...
  // SubscribeAck class
  SubscribeAck::SubscribeAck(uint8_t *data, uint32_t length) : Message(SUBACK),
                                                               _rcs(NULL)
  {
    uint32_t pos = 0;
    _packet_id = read<uint16_t>(data, pos);
  ...
  SubscribeAck::SubscribeAck(Client &client, uint32_t remaining_length) : Message(SUBACK),
                                                                          _rcs(NULL),
  ...
  // Unsubscribe class
  Unsubscribe::Unsubscribe() : Message(SUBSCRIBE),
                               _buffer(NULL), _buflen(0)
  {
    _need_packet_id = true;
  }

  Unsubscribe::Unsubscribe(String topic) : Message(SUBSCRIBE),
                                           _buffer(NULL), _buflen(0)
  {
    _need_packet_id = true;
  ...
  // SubscribeAck class
  UnsubscribeAck::UnsubscribeAck(uint8_t *data, uint32_t length) : Message(UNSUBACK)
  {
    uint32_t pos = 0;
    _packet_id = read<uint16_t>(data, pos);
  }
