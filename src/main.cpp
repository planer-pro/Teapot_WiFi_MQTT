...
NTPClient timeClient(ntpUDP, "europe.pool.ntp.org", 3600 * NTP_TIMEZONE, 60 * NTP_REQ_PERIOD);
...
uint8_t ntpHour;
uint8_t ntpMinute;
uint8_t incomHourValue;
uint8_t incomMinValue;
uint8_t almType = 0;

bool enAlarm = false;
uint8_t modeType = OFF, tryingCnt = 0;
uint16_t an = 0, anOld = 0;
uint32_t _tmTerm = 0, _tmLed = 0, _tmTmp = 0, _tmInf = 0, _tmMqttConn = 0, _tmNtp = 0;
...
void NtpCompareHandler()
{
    if (millis() - _tmNtp > NTP_REQ_PERIOD)
    {
        ...
        {
            ntpHour = tme.substring(0, separatorIndexNtp).toInt(); // Convert to integer
            ntpMinute = tme.substring(separatorIndexNtp + 1).toInt();
        }
